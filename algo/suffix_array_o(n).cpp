        ...
        for(int i = 1; i < n ; i++)
        {
                if(lcp[i] == l){
                        if(((SA[i-1] > len && SA[i] < len) || (SA[i-1] <len && SA[i] >len))){
                                it = st.find(min(SA[i],SA[i-1]));
                                if(it==st.end()){
                                        count++;
                                        st.insert(min(SA[i],SA[i-1]));
                                }
                        }
                }
                else if(lcp[i] > l)
                {
                        for(int j = SA[i] ;(SA[i] + (lcp[i] - l + 1)) < len &&  j < SA[i] + (lcp[i] - l + 1) ; j++ )
                        {
                                it = st.find(j);
                                if(it == st.end())
                                {
